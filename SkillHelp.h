...
#pragma endregion
		...
		System::Void SelectInfoInternal( String^ name )
		{
			...
			else
			{
				...
				if( ability == nullptr )
				{
					if( SpecificAbility::torso_inc && name == SpecificAbility::torso_inc->name )
			...
			else
			{
				...
				while( e.MoveNext() )
				{
					...
					Generic::List< SubSkill^ > skill_subskills;
					...
					for each( SubSkill^ ss in skill_subskills )
					{
					...
					subskills.AddRange( %skill_subskills );
				}
			}
		...
		System::Void cmsDescription_Opening(System::Object^  sender, System::ComponentModel::CancelEventArgs^  e)
		{
			...
			else if( line - armor_list_begin < armors.Count )
			{
				...
				if( preview_pane->SetData( fake_list ) )
				{
					...
					preview_pane->Location = Point( cmsDescription->Location.X + cmsDescription->Size.Width, cmsDescription->Location.Y );
				}
