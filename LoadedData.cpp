...
void DumpMaterials( String^ filename )
{
#ifndef CREATE_MATERIALS
	return;
#endif

	List_t< String^ > mats;
	for each( Material^ mat in Material::static_materials )
	{
		mats.Add( mat->name );
	}
	mats.Sort();
...
void LoadedData::LoadDataFiles()
{
	...
	//DumpSkills( L"Data/Languages/Simplified Chinese/skills.txt" );
	...
	DumpMaterials( L"Data/components.txt" );
...
void GetRelevantDecorations( Query^ query )
{
	...
	if( query->want_taunt )
	{
		for each( Decoration^ decoration in Decoration::static_decorations )
		{
			...
			{
				if( decoration->hr > query->hr && decoration->level > query->level )
					continue;
