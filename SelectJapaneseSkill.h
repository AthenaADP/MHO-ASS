...
namespace MHOASS
{
	...
	public:
		...
		SelectJapaneseSkill(void)
		{
			...
			{
				abilities_by_length[ a->chn_name->Length ]->Add( a );
			}
...
#pragma endregion
	...
	void CalculatePossibleText()
	{
		...
		{
			for( int i = 0; i < ability->chn_name->Length; ++i )
			{
				if( possible_text[ i ]->IndexOf( L"" + ability->chn_name[ i ] ) == -1 )
				{
					const int limit = ( selected_length == 0 ) ? buttons->Length : ability->chn_name->Length;
					bool fail = false;
					for( int j = 0; j < limit; ++j )
					{
						...
						if( buttons[ j ]->Text != L"" && ( j >= ability->chn_name->Length || buttons[ j ]->Text[ 0 ] != ability->chn_name[ j ] ) )
						{
							fail = true;
							break;
						}
					...
					if( !fail)
						possible_text[ i ] += ability->chn_name[ i ];
				}
			}
		}
	}
	...
	void UpdateAbilityList()
	{
		...
		for each( Ability^ ability in abilities_by_length[ selected_length ] )
		{
			...
			for( int i = 0; i < ability->chn_name->Length; ++i )
			{
				...
				if( ability->chn_name[ i ] != buttons[ i ]->Text[ 0 ] )
				{
					fail = true;
					break;
				}
			...
			{
				for( int i = ability->chn_name->Length; i < buttons->Length; ++i )
				{
					if( buttons[ i ]->Text != L"" )
					{
						fail = true;
			...
			if( !fail )
				lstAbilities->Items->Add( ability->chn_name );
		}
	...
	System::Void lstAbilities_SelectedIndexChanged( System::Object^ sender, System::EventArgs^ e )
	{
		if( lstAbilities->SelectedIndex != -1 )
		{
			...
			if( Ability^ a = Ability::FindCharmAbility( lstAbilities->SelectedItem->ToString() ) )
			{
				for( int i = 0; i < a->chn_name->Length; ++i )
					buttons[ i ]->Text = L"" + a->chn_name[ i ];
				for( int i = a->chn_name->Length; i < buttons->Length; ++i )
					buttons[ i ]->Text = L"";
			}
		}
